...
public class MenuRepositories : Repositories<PermissionMenu>, IMenuRepositories
{
  ...
  {
    if (tenancyId != null)
      return await GetDbContext().PermissionMenu.Include(f => f.Permissions)
        .FirstOrDefaultAsync(f => f.Id == id && f.TenancyId == tenancyId);
    return await GetDbContext().PermissionMenu.Include(f => f.Permissions)
      .FirstOrDefaultAsync(f => f.Id == id);
  }

  public async Task<(IList<PermissionMenu> list, int total)> GetEntitiesForPaging(int pageNo, int pageSize,
    string permissionId = "", string title = "", string menuUrl = "",
    long? tenancyId = null)
  {
    var query = new List<PermissionMenu>();
    if (tenancyId != null)
    {
      if (!string.IsNullOrEmpty(permissionId) || !string.IsNullOrEmpty(title) ||
          !string.IsNullOrEmpty(menuUrl))
        query = await GetDbContext().PermissionMenu.Include(f => f.Permissions).Where(w =>
        ...
        query = await GetDbContext().PermissionMenu.Include(f => f.Permissions)
