...
public class AccountController : ControllerBase
{
  ...
  {
    var uid = HttpContext.User.UserId();
    ...
    var permissionList =
      await _serviceProvider.GetRequiredService<PermissionService>().GetPermissionListAsync(permissionIds);
    ...
    var permissionMenuTreeItems = TreeSortMultiLevelFormat(permissionData).ToList();
  ...
  public async Task<Result<AccountResponseDto>> Details([FromQuery] long id)
  {
    ...
    var data = _mapper.Map<AccountResponseDto>(result.Data);
    data.Roles = roles;
  ...
  public async Task<Result> Modify([FromBody] AccountModifyRequestDto dto)
  {
    ...
    if (result.Data.Role != null && result.Data.Role.Count > 0)
    {
      ...
      foreach (var value in result.Data.Role.ToList())
        if (result.Data.Department != null && result.Data.Department.Role.Count > 0 &&
            result.Data.Department.Role.Any(f => f.Id == value.Id))
    ...
    var data = _mapper.Map(dto, result.Data);
    data.SetPassword(dto.Password);
