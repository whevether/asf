...
namespace ASF.Application
{
	...
	public class RoleController: ControllerBase
	{
		...
		{
			long? tenancyId = (HttpContext.User.IsSuperRole() && Convert.ToInt64(HttpContext.User.TenancyId()) == 1) ? null : Convert.ToInt64(HttpContext.User.TenancyId());
			var data = await _serviceProvider.GetRequiredService<RoleService>().GetList(tenancyId,HttpContext.User.IsSuperRole());
			if(!data.Success)
		...
		public async Task<Result> Modify([FromBody] RoleModifyRequestDto dto)
		{
			...
			if (dto.PermissionId != null && dto.PermissionId.Count > 0)
			{
				...
				foreach (string value in dto.PermissionId.ToList())
				{
