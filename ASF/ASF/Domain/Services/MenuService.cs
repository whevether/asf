...
namespace ASF.Domain.Services
{
	...
	public class MenuService
	{
		...
		{
			PermissionMenu permissionMenu = await _menuRepositories.GetPermissionMenuAsync(id,tenancyId);
		...
		public async Task<Result> Modify(PermissionMenu permissionMenu)
		{
			if (await _menuRepositories.GetEntity(f =>
				(f.Id!= permissionMenu.Id && f.TenancyId == permissionMenu.TenancyId && f.Title.Equals(permissionMenu.Title)) || (f.Id != permissionMenu.Id && f.MenuUrl.Equals(permissionMenu.MenuUrl))) != null)
				return Result.ReFailure(ResultCodes.PermissionMenuTitleOrUrlExist);
