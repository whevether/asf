...
public class DepartmentService
{
  ...
  {
    var department = await _departmentRepositories.GetDepartmentAsync(id, tenancyId);
    if (department == null)
  ...
  public async Task<ResultList<Department>> GetList(long? tenancyId = null)
  {
    if (tenancyId != null)
    {
      var list = await _departmentRepositories.GetEntities(f => f.TenancyId == tenancyId);
      if (list == null)
        return ResultList<Department>.ReFailure(ResultCodes.DepartmentNotExist);
      return ResultList<Department>.ReSuccess(list.ToList());
    }

    var data = await _departmentRepositories.GetEntities(f => f.Id != 0);
    if (data == null)
      return ResultList<Department>.ReFailure(ResultCodes.DepartmentNotExist);
  ...
  public async Task<Result> Create(Department department)
  {
    ...
    var isAdd = await _departmentRepositories.Add(department);
    if (!isAdd) return Result.ReFailure(ResultCodes.DepartmentCreateError);
  ...
  public async Task<Result> Modify(Department department)
  {
    if (await _departmentRepositories.GetEntity(f =>
      ...
      return Result.ReFailure(ResultCodes.DepartmentNameExist);
    var isUpdate = await _departmentRepositories.Update(department);
    if (!isUpdate) return Result.ReFailure(ResultCodes.DepartmentUpdateError);
  ...
  {
    var isDelete = await _departmentRepositories.Delete(department);
