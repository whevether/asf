...
public class ApiService
{
  ...
  {
    var api = await _apiRepository.GetApiAsync(id, tenancyId);
    if (api == null)
  ...
  public async Task<ResultList<Api>> GetList(List<string> ids, long? tenancyId)
  {
    ...
    if (tenancyId != null)
    {
      var list = await _apiRepository.GetEntities(f =>
        f.IsSystem != null && (Status)f.IsSystem != Status.Yes && ids.Any(x => x.Equals(f.Id.ToString())) &&
  ...
  public async Task<(IList<Api> list, int total)> GetList(int pageNo, int pageSize, string permissionId = "",
    ...
    long? tenancyId = null)
  {
    return await _apiRepository.GetEntitiesForPaging(pageNo, pageSize, permissionId, type, status, name, httpMethod,
      path, tenancyId);
  }
  ...
  public async Task<Result> Create(Api api)
  {
    ...
    if (!isAdd) return Result.ReFailure(ResultCodes.PermissionApiCreateError);
  ...
  {
    if (await _apiRepository.GetEntity(f =>
          f.Id != api.Id && f.TenancyId == api.TenancyId && f.Path.Equals(api.Path)) != null)
      return Result.ReFailure(ResultCodes.PermissionApiPathExist);
    var isUpdate = await _apiRepository.Update(api);
    if (!isUpdate) return Result.ReFailure(ResultCodes.PermissionApiUpdateError);
    return Result.ReSuccess();
  }
  ...
  {
    var isdDelete = await _apiRepository.Delete(api);
    if (!isdDelete)
  ...
  {
    var isdDelete = await _apiRepository.DeleteRange(list);
    if (!isdDelete)
      return Result.ReFailure(ResultCodes.PermissionSysApiDeleteError);
    return Result.ReSuccess();
  }
