...
namespace ASF.Domain.Services
{
	...
	public class TenancyService
	{
		...
		/// <returns></returns>
		public async Task<(IList<Tenancy> list, int total)> GetList(int pageNo, int pageSize, string name = "", uint? status = null,long? tenancyId = null)
		{

			if (!string.IsNullOrEmpty(name) && status != null && tenancyId != null)
			{
				var (list,total) = await _repository.GetEntitiesForPaging(pageNo, pageSize, f => f.Name.Equals(name) && f.Status == status && (Status)f.IsDeleted == Status.No && f.Id == tenancyId);
				return (list,total);
			}
			
			if (!string.IsNullOrEmpty(name) && tenancyId != null)
			{
				var (list,total) = await _repository.GetEntitiesForPaging(pageNo, pageSize, f => f.Name.Equals(name)&& (Status)f.IsDeleted == Status.No && f.Id == tenancyId);
				return (list,total);
			}

			if (status != null && tenancyId != null)
			{
				var (list,total) = await _repository.GetEntitiesForPaging(pageNo, pageSize, f => f.Status == status&& (Status)f.IsDeleted == Status.No && f.Id == tenancyId);
				return (list,total);
			}

			if (tenancyId != null)
			{
				var (list,total) = await _repository.GetEntitiesForPaging(pageNo, pageSize, f => f.Id != 0&& (Status)f.IsDeleted == Status.No && f.Id == tenancyId);
				return (list,total);
			}
