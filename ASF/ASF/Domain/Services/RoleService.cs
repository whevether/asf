	...
	public class RoleService
	{
		...
		public async Task<ResultList<Role>> GetList(long? tenancyId = null,bool isSuperAdmin = false)
		{
			if (tenancyId != null)
			{
				if (isSuperAdmin)
				{
					IEnumerable<Role> list = await _roleRepositories.GetEntities(f => f.TenancyId == tenancyId);
					return ResultList<Role>.ReSuccess(list.ToList());
				}
				else
				{
					IEnumerable<Role> list = await _roleRepositories.GetEntities(f => f.TenancyId == tenancyId && !f.Name.Equals("superadmin"));
					return ResultList<Role>.ReSuccess(list.ToList());
				}
			}
			else
			{
				if (isSuperAdmin)
				{
					IEnumerable<Role> list = await _roleRepositories.GetEntities(f => f.Id != 0);
					return ResultList<Role>.ReSuccess(list.ToList());
				}
				else
				{
					IEnumerable<Role> list = await _roleRepositories.GetEntities(f => !f.Name.Equals("superadmin"));
					return ResultList<Role>.ReSuccess(list.ToList());
				}
			}
		}
		...
		{
			//判断是否存在相同的角色名称
			if (await _roleRepositories.GetEntity(f => f.TenancyId == role.TenancyId && f.Id != role.Id &&f.Name.Equals(role.Name)) != null)
				return Result.ReFailure(ResultCodes.RoleNameExist);
		...
		{
			bool isDelete = await _roleRepositories.Delete(role);
			if (!isDelete)
			{
