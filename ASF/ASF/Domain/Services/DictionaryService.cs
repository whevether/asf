...
{
  private readonly IAsfDictionaryRepository _asfDictionaryRepository;
  private readonly IIdGenerator _idGenerator;

  /// <summary>
  ///   字典领域服务
  /// </summary>
  public DictionaryService(IAsfDictionaryRepository asfDictionaryRepository, IIdGenerator idGenerator)
  {
  ...
  public async Task<Result<AsfDictionary>> Get(long id, long? tenancyId = null)
  {
    if (tenancyId != null)
    {
      var d = await _asfDictionaryRepository.GetEntity(f => f.Id == id && f.TenancyId == tenancyId);
      if (d == null)
        return Result<AsfDictionary>.ReFailure(ResultCodes.DictionaryNotExist);
      return Result<AsfDictionary>.ReSuccess(d);
    }

    var asfDictionary = await _asfDictionaryRepository.GetEntity(f => f.Id == id);
    ...
    long? tenancyId = null)
  {
    ...
    {
      var (list, total) =
        await _asfDictionaryRepository.GetEntitiesForPaging(pageNo, pageSize,
          f => f.Key.Equals(key) && f.TenancyId == tenancyId);
      return (list, total);
    }
    ...
    {
      var (list, total) =
        await _asfDictionaryRepository.GetEntitiesForPaging(pageNo, pageSize, f => f.TenancyId == tenancyId);
      return (list, total);
    }
  ...
  public async Task<ResultList<AsfDictionary>> GetList(long? tenancyId = null)
  {
    if (tenancyId != null)
    {
      var l = await _asfDictionaryRepository.GetEntities(f => f.Id != 0 && f.TenancyId == tenancyId);
      if (l == null)
    ...
    var list = await _asfDictionaryRepository.GetEntities(f => f.Id != 0);
    if (list == null)
      return ResultList<AsfDictionary>.ReFailure(ResultCodes.DictionaryNotExist);
  ...
  public async Task<Result> Create(AsfDictionary asfDictionary)
  {
    if (await _asfDictionaryRepository.GetEntity(f =>
          f.TenancyId == asfDictionary.TenancyId && f.Key.Equals(asfDictionary.Key)) != null)
      return Result.ReFailure(ResultCodes.DictionaryKeyExist);
    asfDictionary.SetId(_idGenerator.GenId());
    var isAdd = await _asfDictionaryRepository.Add(asfDictionary);
    if (!isAdd) return Result.ReFailure(ResultCodes.DictionaryCreateError);
  ...
  public async Task<Result> Modify(AsfDictionary asfDictionary)
  {
    ...
    if (!isUpdate) return Result.ReFailure(ResultCodes.DictionaryUpdateError);
  ...
  {
    var isDelete = await _asfDictionaryRepository.Delete(asfDictionary);
    if (!isDelete) return Result.ReFailure(ResultCodes.DictionaryDeleteError);
