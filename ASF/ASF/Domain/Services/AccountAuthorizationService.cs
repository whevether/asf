...
namespace ASF.Domain.Services
{
  ...
  public class AccountAuthorizationService
  {
    ...
    public AccountAuthorizationService(IHttpContextAccessor httpContextAccessor, IServiceProvider serviceProvider, ILogger<AccountAuthorizationService> logger)
    {
    ...
    public async Task<Result<Api>> Authentication()
    {
      ...
      if ((account.Department != null && account.Department.Role.Count != 0 && account.Department.Role.Any(f => (f.Enable != null && (EnabledType)f.Enable == EnabledType.Disabled) || f.Permission.Count(x => (x.Enable != null && (EnabledType)x.Enable == EnabledType.Enable) && x.Id == api.PermissionId) == 0)) 
          && 
          (account.Role.Count != 0 && account.Role.Any(f => (f.Enable != null && (EnabledType)f.Enable == EnabledType.Disabled) || f.Permission.Count(x => (x.Enable != null && (EnabledType)x.Enable == EnabledType.Enable) && x.Id == api.PermissionId) == 0)))
      {
    ...
    private async Task<Api> MatchApiPermission(string requestPath)
    {
      ...
      IEnumerable<Api> api = await service.GetEntities(f => f.Id != 0);
