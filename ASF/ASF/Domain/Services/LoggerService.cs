...
namespace ASF.Domain.Services
{
  ...
  public class LoggerService
  {
    ...
    public async Task<ResultList<LogInfo>> GetList(List<string> ids)
    {
      ...
      IEnumerable<LogInfo> list = await _loggingsRepository.GetEntities(f => ids.Any(x => x.Equals(f.Id.ToString())));
      if (list == null)
        return ResultList<LogInfo>.ReFailure("没有日志数据", 3500);
    ...
    public async Task<Result> Delete(List<LogInfo> logInfo)
    {
      ...
      bool isDelete = await _loggingsRepository.DeleteRange(logInfo);
      if (!isDelete)
      {
