...
namespace ASF.Domain.Services
{
	...
	public class PermissionService
	{
		...
		{
			Permission permission = await _permissionsRepository.GetAsync(id,tenancyId);
			if (permission == null)
		...
		public async Task<ResultList<Permission>> GetList(long? tenancyId,bool isSuperAdmin = false)
		{
			if (tenancyId != null)
			{
				...
				{
					IEnumerable<Permission> list = await _permissionsRepository.GetEntities(f => f.Id != 0 && f.IsSystem != null && f.IsSystem != 1);
					return ResultList<Permission>.ReSuccess(list.ToList());
				}
			}
			else
		...
		/// <param name="tenancyId"></param>
		/// <returns></returns>
		public async Task<(IList<Permission> list, int total)> GetList(int pageNo, int pageSize,string code="",string name = "",uint? type = null,uint? isSys = null,uint? status = null,long? tenancyId = null )
		{
			if (!string.IsNullOrEmpty(code) && !string.IsNullOrEmpty(name) && type != null  &&
			    status != null && tenancyId != null)
			{
				var (list, total) =
					await _permissionsRepository.GetEntitiesForPaging(pageNo, pageSize, f => f.Code.Contains(code) && f.Name.Contains(name) && f.Type == type && f.Enable == status && f.TenancyId == tenancyId);
				return (list,total);
			}
			...
			{
				var (list, total) =
					await _permissionsRepository.GetEntitiesForPaging(pageNo, pageSize, f => f.Name.Contains(name) && f.Type == type && f.IsSystem != 1 && f.Enable == status && f.TenancyId == tenancyId);
				return (list,total);
			}
			if (type != null  &&
			    status != null && tenancyId != null)
			{
				var (list, total) =
					await _permissionsRepository.GetEntitiesForPaging(pageNo, pageSize, f =>f.Type == type && f.IsSystem != 1 && f.Enable == status && f.TenancyId == tenancyId);
				return (list,total);
			}
			if (
			    status != null && tenancyId != null)
			{
				var (list, total) =
					await _permissionsRepository.GetEntitiesForPaging(pageNo, pageSize, f => f.IsSystem != 1 && f.Enable == status && f.TenancyId == tenancyId);
				return (list,total);
			}
			if (
			...
			{
				var (list, total) =
					await _permissionsRepository.GetEntitiesForPaging(pageNo, pageSize, f =>f.Code.Contains(code)&& f.TenancyId == tenancyId );
				return (list,total);
			}
			if (
			...
			{
				var (list, total) =
					await _permissionsRepository.GetEntitiesForPaging(pageNo, pageSize, f =>f.Name.Contains(name)&& f.TenancyId == tenancyId );
				return (list,total);
			}
			if (type != null && tenancyId != null)
			{
				var (list, total) =
					await _permissionsRepository.GetEntitiesForPaging(pageNo, pageSize, f =>f.Type == type&& f.TenancyId == tenancyId);
				return (list,total);
			}
			if (tenancyId != null)
			{
			...
			{
				var (list, total) =
					await _permissionsRepository.GetEntitiesForPaging(pageNo, pageSize, f => f.Code.Contains(code) && f.Name.Contains(name) && f.Type == type && f.IsSystem == isSys && f.Enable == status);
				return (list,total);
			}
			...
			{
				var (list, total) =
					await _permissionsRepository.GetEntitiesForPaging(pageNo, pageSize, f => f.Name.Contains(name) && f.Type == type && f.IsSystem == isSys && f.Enable == status);
				return (list,total);
			}
			...
			{
				var (list, total) =
					await _permissionsRepository.GetEntitiesForPaging(pageNo, pageSize, f =>f.Type == type && f.IsSystem == isSys && f.Enable == status);
				return (list,total);
			}
			if (isSys != null &&
			    status != null)
			{
				var (list, total) =
					await _permissionsRepository.GetEntitiesForPaging(pageNo, pageSize, f => f.IsSystem == isSys && f.Enable == status);
				return (list,total);
			}
			if (
			    status != null)
		...
		{
			if (await _permissionsRepository.GetEntity(f => f.TenancyId == permission.TenancyId && (f.Code.Equals(permission.Code) || f.Name.Equals(permission.Name))) != null)
		...
		public async Task<Result> Modify(Permission permission)
		{
			...
			if (await _permissionsRepository.GetEntity(f => (f.Id != permission.Id &&f.TenancyId == permission.TenancyId &&(f.Name.Equals(permission.Name) || f.Code.Equals(permission.Code)))) != null)
				return Result.ReFailure(ResultCodes.PermissionNameOrCodeExist);
