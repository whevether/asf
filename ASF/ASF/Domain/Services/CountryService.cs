...
namespace ASF.Domain.Services
{
	...
	public class CountryService
	{
		...
		{
			Country country = await _countryRepositories.GetEntity(f => f.Id == id);
			if (country == null)
		...
		public async Task<(IList<Country> list,int total)> GetList(int pageNo, int pageSize, string name)
		{
			...
			{
				var (list,total) = await _countryRepositories.GetEntitiesForPaging(pageNo, pageSize, f => f.Name.Contains(name) || f.LanguageCode.Contains(name));
				return (list,total);
			}
		...
		public async Task<ResultList<Country>> GetList()
		{
			...
			IEnumerable<Country> list = await _countryRepositories.GetEntities(f => f.Id != 0);
			if(list == null)
		...
		{
			if (await _countryRepositories.GetEntity(f =>f.Name.Equals(country.Name) || f.LanguageCode.Equals(country.LanguageCode)) != null)
				return Result.ReFailure(ResultCodes.CountryNameExistError);
			country.SetId(_idGenerator.GenId());
			bool isAdd = await _countryRepositories.Add(country);
			if (!isAdd)
			{
				return Result.ReFailure(ResultCodes.CountryCreateError);
			}
		...
		{
			if (await _countryRepositories.GetEntity(f =>f.Id !=country.Id && (f.Name.Equals(country.Name) || f.LanguageCode.Equals(country.LanguageCode))) != null)
				return Result.ReFailure(ResultCodes.CountryNameExistError);
			bool isUpdate = await _countryRepositories.Update(country);
			if (!isUpdate)
			{
				return Result.ReFailure(ResultCodes.CountryUpdateError);
			}
		...
		{
			bool isDelete = await _countryRepositories.Delete(country);
			if (!isDelete)
			{
				return Result.ReFailure(ResultCodes.CountryDeleteError);
			}
